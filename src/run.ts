...
async function run(terminal, config, rootPath?, substitutions?) {
  ...
  const { name, color, icon, targets, split, cwd: terminalCwd, command, commands, execute, persistent, recycle, substitution, dynamicTitle, shellPath, env: terminalEnv, envInherit } = terminal,
  ...
  var _term = null;
  vscode.window.showErrorMessage ( targets );
  if (targets.length < 1) {
    const cacheTarget = targets[0] || name,
    ...
    for (var target in targets) {
      ...
      var cacheTarget = targets[target] || name,
        cacheTerm = recycle !== false && cache[cacheTarget],
        cacheParentTerm = split && cache[split],
        location = cacheParentTerm ? { parentTerminal: cacheParentTerm } : undefined,
        title = dynamicTitle ? undefined : cacheTarget,
